...
def test_subscription_detection_netflix_pattern():
    ...
    assert result_180d["monthly_recurring_spend"] == pytest.approx(
        expected_monthly, abs=0.5
...
def test_credit_utilization_calculation():
    ...
    assert result["max_utilization_pct"] == pytest.approx(
        expected_utilization, abs=0.1
...
def test_emergency_fund_coverage():
    ...
    assert result["emergency_fund_months"] == pytest.approx(
        3.0, abs=1.0
...
def test_edge_case_no_transactions():
    ...
    empty_liabilities = pd.DataFrame(
        columns=[
            "liability_id",
            "account_id",
            "user_id",
    ...
    sav_result = calculate_savings_signals(
        empty_transactions, empty_accounts, user_id, window_days=30
    )
...
def test_full_feature_pipeline(temp_data_dir):
    ...
    assert (
        "sav_180d_emergency_fund_months" in signals_df.columns
    ), "Should have 180d savings signals"
