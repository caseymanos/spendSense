...
def render_data_generation_tab():
    ...
    with ui.card().classes(ThemeManager.get_card_classes()):
        ...
        ui.label("Configure parameters and generate synthetic test data").classes(
            "text-sm text-gray-600 mb-6"
        )
        ...
        with ui.column().classes("gap-6 w-full"):
            ...
            with ui.row().classes("items-center gap-4 w-full"):
                ...
                seed_slider = (
                    ui.slider(min=0, max=9999, value=current_config.get("seed", 42), step=1)
                    .classes("flex-grow")
                    .props("label-always")
                )
                seed_input = (
                    ui.number(value=current_config.get("seed", 42), min=0, max=9999)
                    .classes("w-32")
                    .bind_value(seed_slider, "value")
                )
            ...
            with ui.row().classes("items-center gap-4 w-full"):
                ...
                users_slider = (
                    ui.slider(min=10, max=1000, value=current_config.get("num_users", 100), step=10)
                    .classes("flex-grow")
                    .props("label-always")
                )
                users_input = (
                    ui.number(value=current_config.get("num_users", 100), min=10, max=1000)
                    .classes("w-32")
                    .bind_value(users_slider, "value")
                )
            ...
            with ui.row().classes("items-center gap-4 w-full"):
                ...
                months_slider = (
                    ui.slider(min=1, max=24, value=current_config.get("months_history", 6), step=1)
                    .classes("flex-grow")
                    .props("label-always")
                )
                months_input = (
                    ui.number(value=current_config.get("months_history", 6), min=1, max=24)
                    .classes("w-32")
                    .bind_value(months_slider, "value")
                )
            ...
            with ui.row().classes("items-center gap-4 w-full"):
                ...
                trans_slider = (
                    ui.slider(
                        min=10,
                        max=100,
                        value=current_config.get("avg_transactions_per_month", 30),
                        step=5,
                    )
                    .classes("flex-grow")
                    .props("label-always")
                ...
                trans_input = (
                    ui.number(
                        value=current_config.get("avg_transactions_per_month", 30), min=10, max=100
                    )
                    .classes("w-32")
                    .bind_value(trans_slider, "value")
                )
        ...
        with ui.card().classes("bg-gray-50 p-4"):
            ...
            config_preview = (
                ui.json_editor(preview_config).props("mode=view readonly").classes("w-full h-48")
            )
            ...
            est_transactions = (
                int(users_slider.value) * int(months_slider.value) * int(trans_slider.value)
            )
            est_label = ui.label(f"Estimated transactions: {est_transactions:,}").classes(
                "text-sm text-gray-600 mt-2"
            )
        ...
        with ui.row().classes("gap-4"):
            ...
            ui.button("Generate Data", icon="play_arrow", on_click=generate_data).props(
            ...
            ui.button("Load from Config", icon="upload", on_click=load_from_config).props(
                "flat"
            ).classes(ThemeManager.get_button_classes())
