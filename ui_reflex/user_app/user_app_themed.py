...
def navbar(theme_config) -> rx.Component:
    ...
    return rx.box(
        rx.hstack(
            ...
            # User info
            rx.hstack(
                rx.text(UserAppState.user_name, font_weight="500", color=theme.text_primary),
                rx.badge(
...
def persona_badge_themed(persona_info: dict, theme_config) -> rx.Component:
    ...
    theme = theme_config.colors
    persona_color = get_persona_color(theme_config, persona_info.get("id", "general"))

    return rx.box(
        rx.vstack(
            rx.heading(
                f"🎯 Your Persona: {persona_info.get('name', 'Unknown')}",
                size="6",
                color=theme.text_primary,
                margin_bottom="3",
            ),
...
def recommendation_card_themed(rec: dict, theme_config) -> rx.Component:
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.text(rec["description"], color=theme.text_secondary, margin_bottom="2"),
            rx.box(
                rx.text(
                    "💡 " + rec["rationale"],
                    font_style="italic",
                ...
                rx.cond(
                    ...
                    rx.vstack(
                        ...
                        rx.cond(
                            ...
                            rx.text("No recommendations available", color=theme.text_muted),
                        ),
                        spacing="4",
                        width="100%",
                    ),
